(function () {
  ...
  window.posRequest = async function posRequest(apiPath, options = {}) {
    ...
    const html = String(last || "").trim().startsWith("<");
    throw new Error(
      html
        ? "Sign-in did not reach the POS server (got a web page). After this deploy, open /api/health — it must be JSON. hPanel: Node.js app, Express, entry server.js, empty build/output, Restart."
        : "Could not reach the POS API. Restart the Node.js app in hPanel (entry server.js).",
    );
  };
