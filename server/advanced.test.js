...
test("PHP and Node wire barcode, damage, loyalty, and ledger modules", () => {
  ...
  assert.match(nodeAdv, /body\.barcode_qty/);
  assert.match(adv, /barcode_qty/);
  ...
  assert.doesNotMatch(index, /Manufacturer barcode/);
  assert.doesNotMatch(index, /id="item-barcode"/);
  assert.doesNotMatch(index, /id="item-mfr-barcode"/);
  ...
  assert.match(app, /kind !== "own"/);
  assert.match(app, /kind !== "manufacturer"/);
  assert.match(app, /pcs-barcode-only/);
...
test("item save and checkout persist barcode and line discount fields", () => {
  ...
  assert.match(nodeAdv, /export async function reverseLoyaltyOnSale/);
  assert.match(adv, /function pos_loyalty_reverse_sale/);
