...
function clipKotLines(raw) {
  ...
  return list
    ...
    .map((line) => ({
      ...
      notes: String(line?.notes || line?.special_instruction || line?.specialInstruction || "").trim().slice(0, 240),
    }))
...
export function registerTenant(app) {
  ...
  app.get("/api/stock/excel", requireStaff, requirePerm("stock"), async (_req, res) => {
    try {
      ...
      try {
        batches = await query(
          `SELECT b.id, b.item_id, b.batch_no, b.barcode, b.qty_gm, b.remaining_gm, b.unit_cost, b.mrp,
                  ...
                  i.name AS item_name, i.code AS item_code, i.base_unit, i.unit, i.purchase_rate, i.retail_rate, i.b2b_rate, i.gst_rate,
      ...
      const xml = workbookXml(stockToSheets(rows, biz, batches));
